...
export default function EditorOvercast({isOpen, oldTitle, oldDescription, isDeletable, callback}) {
  ...
  return (
    <div ref={bgEleRef} onClick={cancelClickHandler} className={clsx("absolute top-0 left-0 z-10 size-full [background-color:rgba(25,25,25,0.7)] flex flex-row items-center",
      ...
      <div className={"w-96 h-96 px-8 py-8 bg-primary m-auto"}>
        <button onClick={xClickHandler}
                className={"btn btn-square btn-outline border-0 bg-primary h-8 min-h-8 w-8 text-base-100 float-end -mr-6 -mt-6"}>
          <XMarkIcon className={""}/>
        </button>
        ...
        <button onClick={applyClickHandler} disabled={title.length <= 0}
                className={clsx("btn h-8 min-h-8 -mr-4 btn-base-100 float-end hover:btn-secondary disabled:text-primary")}>确定
        </button>
        <button onClick={deleteClickHandler}
                className={clsx("btn btn-square btn-outline mr-2 border-0 float-end bg-primary h-8 min-h-8 w-8 text-base-100 float-end hover:bg-primary hover:text-red-700",
                  { "hidden": !isDeletable })}>
          <TrashIcon className={""}/>
        </button>
      </div>
